...
export const TreeGeometry = {
  ...
  calculateSegmentLength(branch) {
    ...
    const self = this;
    branch.children.forEach(function (child) {
      const len = self.calculateSegmentLength(child);
      if (len > longest) {
        longest = len;
      }
    });

...
const init = () => {
  ...
  camera = new PerspectiveCamera(30, window.innerWidth / window.innerHeight, 1, 500);
...
export const animate = () => {
  ...
  if (options.flyAround) {
    const radius = 200;
    camera.position.x = tree.position.x + radius * Math.cos(0.4 * clock.elapsedTime);
    camera.position.z = tree.position.z + radius * Math.sin(0.4 * clock.elapsedTime);
    camera.position.y = 90 + (tree.position.y + 35 * Math.sin(0.2 * clock.elapsedTime));
...
function makeTree(length, pos, rz, ry, rBottom = length / 20) {
  ...
  for (let i = 0; i < numberOfBranches; i++) {
    rz = MathUtils.degToRad(randomFloatFromInterval(-options.maxBranchAngle, options.maxBranchAngle));
    ry = MathUtils.degToRad(randomFloatFromInterval(-options.maxBranchAngle, options.maxBranchAngle));
...
function getNumberOfBranches() {
  ...
  return rnd < 1 - options.branchDensity ? randomIntFromInterval(1, 2) : randomIntFromInterval(3, 4);
}
...
function addLeaf(group) {
  ...
  const leafColour = new Color()
    ...
    .offsetHSL(MathUtils.seededRandom() / 10, MathUtils.seededRandom() / 10, MathUtils.seededRandom() / 8);
  ...
  const rotationParent = new Group();
...
function createGUI() {
  ...
  leavesFolder.open();
  treeFolder.add(options, "seed").name("Seed").listen().onFinishChange(createTree);
  treeFolder.add(options, "height", 10, 30, 5).name("Height").onFinishChange(createTree);
  treeFolder
    .add(options, "minBranchLength", minBranchLengthRange[0], minBranchLengthRange[1], 1)
    .name("Min. Branch Length")
    .onFinishChange(createTree);
  treeFolder.add(options, "maxBranchAngle", 30, 180, 5).name("Max. Angle").onFinishChange(createTree);
  treeFolder.add(options, "branchDensity", 0.1, 0.5, 0.1).name("Branch Density").onFinishChange(createTree);
  treeFolder.addColor(options, "treeColour").name("Colour").onFinishChange(createTree);

  leavesFolder.add(options, "hasLeaves").name("Show?").onFinishChange(createTree);
  leavesFolder.add(options, "leafSize", 10, 40, 5).name("Size").onFinishChange(createTree);
  leavesFolder.addColor(options, "leafColour").name("Colour").onFinishChange(createTree);
